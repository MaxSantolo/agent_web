...
$email2check = $conn->query("SELECT email FROM email_check limit 100");
...
$quante_email = count($emails)-1; //eliminare ultima iterazione dell'array generato da mysqli_fetch_assoc;
    ...
    for($f=0;$f<$quante_email;$f++) {
        ...
        if ($email != NULL) {
            $result = $apiInstance->getContactStats($email)->getOpened();
        ...
        for ($i = 0; $i < $quante_azioni; $i++) {
            ...
            $conn->query("INSERT INTO campaigns_react (email, id_campagna, reazione, data) VALUES ('" . $email . "', '" . $id_campagne . "', 'Aperta', '" . $data_evento . "')");
        }
    }

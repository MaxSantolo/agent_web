...
$email2check = $conn->query("SELECT email FROM email_check UNION SELECT email from email_check_blacklisted LIMIT 1");
...
$quante_email = count($emails)-1; //eliminare ultima iterazione dell'array generato da mysqli_fetch_assoc
    ...
    for($f=0;$f<$quante_email;$f++) {
        ...
        if ($email != NULL) {
            $result = $apiInstance->getContactStats($email)->getUnsubscriptions()->getUserUnsubscription();
        ...
        if ($result != NULL) {
            ...
            //for ($i = 0; $i < $quante_azioni; $i++) {
                ...
                $id_campagne = $result[0]["campaignId"];
                $data_evento = $result[0]["eventTime"]->format('Y-m-d');

                $conn->query("INSERT INTO campaigns_react (email, id_campagna, reazione, data) VALUES ('" . $email . "', '" . $id_campagne . "', 'Disiscrizione', '" . $data_evento . "')");
                }
            //}
    }
